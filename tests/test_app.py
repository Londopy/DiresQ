...
class TestTheDocsAreNotOutOfDate:
    ...
    def walk(self):
        """Every hand-written file, once.

        os.walk with the skip list pruned out of `dirnames`, rather than
        rglob("*") followed by a filter — the filter still descends into
        node_modules and reads several thousand paths to throw them away,
        which made this class the slowest thing in the suite.
        """
        import os
        for here, dirnames, filenames in os.walk(diresq.SCHEMA.parent):
    ...
    def code_lines(self):
        return self.count(p for p in self.walk()
                          if p.suffix in self.CODE_SUFFIXES)

    # Files that exist but nobody typed. package-lock.json is npm's, the model
    # is generated by `export-model`, and site/src/pages is a copy of docs/
    # made at build time — counting any of them as work would be counting the
    # documentation twice and a lockfile once.
    GENERATED = {"package-lock.json", "priority.json"}
    PROSE_SUFFIXES = {".md", ".yml", ".yaml", ".cff", ".txt"}

    def prose_lines(self):
        """Documentation and configuration — everything hand-written that
        isn't code."""
        def keep(p):
            if p.suffix not in self.PROSE_SUFFIXES:
                return False
            if p.name in self.GENERATED:
                return False
            # The docs site mirrors docs/ into site/src/pages at build time.
            return not ("site" in p.parts and "pages" in p.parts)

        return self.count(p for p in self.walk() if keep(p))
    ...
    def test_the_written_total_is_true(self):
        """Code plus prose. The number most likely to be quoted at somebody
        and the one nothing else in this class would catch."""
        claimed = re.findall(r"\*\*([\d,]+) lines written\*\*",
                             self.live("README.md"))
        assert claimed, "the README no longer states a written total"
        actual = self.code_lines() + self.prose_lines()
        for number in claimed:
            self.assert_close(int(number.replace(",", "")), actual,
                              "README lines written")

    def test_the_written_total_is_not_padded_with_generated_files(self):
        # 5,481 of those lines were npm's lockfile the first time this was
        # measured. A total that counts a lockfile is not a claim about work.
        assert self.prose_lines() < 8_000, (
            "prose count looks inflated — a generated file has crept back in")
